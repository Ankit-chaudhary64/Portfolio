...
export default function PublicationsFooterSection() {
  ...
  const videoEndedThisVisitRef = useRef(false)
  ...
  useEffect(() => {
    ...
    if (!isMobile && canvas && videoEl) {
      ...
      const initialAspect = videoEl.videoWidth && videoEl.videoHeight
      ...
      vidUni = {
        uVideo:       { value: vidTex },
        uOpacity:     { value: 0 },
        uVideoAspect: { value: initialAspect },
      ...
      videoEl.addEventListener('ended', () => {
        videoEndedThisVisitRef.current = true
        vidUni.uOpacity.value = 0
        syncVideoState()
      })
      ...
      syncVideoState = () => {
        const shouldRun = !videoEndedThisVisitRef.current && shouldShowVideo && sectionVisible && pageVisible
    ...
    function onScroll() {
      ...
      if (isMobile) {
        ...
        shouldShowVideo = !videoEndedThisVisitRef.current && xfade > 0.04

        if (videoEndedThisVisitRef.current) {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        if (sectionVisible) {
          if (!wasVisible) {
            videoEndedThisVisitRef.current = false
            if (videoEl && videoEl.currentTime >= videoEl.duration) videoEl.currentTime = 0
          }
          onScroll()
        } else {
          if (videoEndedThisVisitRef.current && videoEl) {
            videoEl.currentTime = 0
          }
          syncVideoState()
        }
          ...
          <div ref={bottomBarRef} className={styles.bottomBar}>
            <div className={styles.bottomLeft}>
              <div className={styles.monogram}>
                <span className={styles.monoLetters}>AC</span>
