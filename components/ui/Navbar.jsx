...
export default function Navbar() {
  ...
  const time = useSyncExternalStore(subscribeClock, getIST, () => '')
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const nextOnIntro = currentY < vh * 0.8
      const nextOnDark = sectionIdx >= 3
      setOnIntro(prev => (prev !== nextOnIntro ? nextOnIntro : prev))
      setOnDark(prev => (prev !== nextOnDark ? nextOnDark : prev))
  ...
  return (
    <>
      <header ref={headerRef} className={`${styles.header} ${onIntro ? styles.introMode : ''} ${onDark ? styles.darkMode : ''}`}>
        <span className={styles.time}>INDIA TIME (IST) - {time}</span>
